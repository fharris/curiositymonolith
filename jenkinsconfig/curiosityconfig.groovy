      ...
      steps {
        withKubeConfig( credentialsId: 'jenkins-token-kubernetes', serverUrl: kubernetes_proxy ) {
           ...
           // sh "kubectl -n curiositymonolith delete secret curiositymonolith-mysql-db-secret"
            sh "kubectl -n curiositymonolith exec -it `kubectl get pods -n curiositymonolith|grep mysql|awk '{print $1}'` -- mysql -h 127.0.0.1 -u root -pmySQLpword#2023 < ./databaseconfig/create-curiositydb-resources.sql"
            sh "kubectl -n curiositymonolith exec -it mysql-db-deployment-84c7bc9b94-cnvkb -- mysql -h 127.0.0.1 -u root -pmySQLpword#2023 < ./databaseconfig/create-curiositydb-resources.sql"
